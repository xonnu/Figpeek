...
<main class="main flex flex-col gap-4">
  ...
  <div class="flex flex-col gap-2">
    ...
    <input maxlength="25" class="w-[300px] border rounded-md py-2 px-4 text-sm" type="text" id="title" bind:value={title} on:change={capture}>
  </div>

  <div class="flex flex-col gap-2">
    <label class="text-sm" for="description">Project description</label>
    <input class="w-[300px] border rounded-md py-2 px-4 text-sm" type="text" id="description" bind:value={(description)} on:change={capture}>
  </div>
...
<div class="w-full bg-white flex items-center justify-center pointer-events-none select-none pb-24 pt-10">
  <div id="capture" class="rounded-xl overflow-hidden px-[100px] flex flex-col gap-4 items-start justify-center w-[1240px] h-[640px] transform text-white" style:background-color={color}>
    <h2 class="block font-bold text-7xl whitespace-normal text-left ml-24 w-[700px] leading-normal" on:change={capture}>{title || 'Figma Thumbnail Generator ✨'}</h2>
    <p class="block mt-7 text-4xl whitespace-normal w-full text-left ml-24">{description || 'Welcome to Figma Thumbnail Generator'}</p>
  </div>
</div>
